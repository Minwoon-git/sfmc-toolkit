...
def call_ai(system_prompt: str, user_message: str) -> str:
    """Anthropic → OpenAI → Gemini 순으로 사용 가능한 키로 호출"""
    if active_ai:
        import anthropic
...
출력 형식:
SQL 코드만 출력하세요. 마크다운 코드펜스(```), 주석, 설명 텍스트 일절 포함하지 마세요. 순수 SQL만."""
        ...
        with tab_dash:
            ...
            if not st.session_state.get("sfmc_token"):
                ...
                if stats:
                    ...
                    with chart_col:
                        if chart_stats:
                            names = [s["name"][:18]+"…" if len(s["name"])>18 else s["name"] for s in chart_stats]
                            ok_data = [s["sends"]-s["bounces"] for s in chart_stats]
                            bounce_data = [s["bounces"] for s in chart_stats]
                            chart_h = max(220, len(chart_stats) * 44 + 60)
                            st.markdown(f"""<div style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:16px;">
                            <div style="font-size:13px;font-weight:500;color:#1e293b;margin-bottom:4px;">Journey별 발송 현황</div>
                            <div style="display:flex;gap:12px;margin-bottom:10px;font-size:11px;">
                            <span style="display:flex;align-items:center;gap:4px;"><span style="width:10px;height:10px;border-radius:2px;background:#bfdbfe;display:inline-block;"></span><span style="color:#64748b;">정상 발송</span></span>
                            <span style="display:flex;align-items:center;gap:4px;"><span style="width:10px;height:10px;border-radius:2px;background:#fecaca;display:inline-block;"></span><span style="color:#64748b;">바운스</span></span>
                            </div>
                            <div style="position:relative;height:{chart_h}px;">
                            <canvas id="sendBarChart"></canvas>
                            </div></div>
                            <script>
                            (function() {{
                              var src = 'https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js';
                              function drawBar() {{
                                var el = document.getElementById('sendBarChart');
                                if (!el) return;
                                if (window._sendChart) {{ window._sendChart.destroy(); window._sendChart = null; }}
                                window._sendChart = new Chart(el, {{
                                  type: 'bar',
                                  data: {{
                                    labels: {_json.dumps(names, ensure_ascii=False)},
                                    datasets: [
                                      {{label:'정상 발송', data:{_json.dumps(ok_data)}, backgroundColor:'#bfdbfe', borderRadius:3, stack:'s'}},
                                      {{label:'바운스', data:{_json.dumps(bounce_data)}, backgroundColor:'#fecaca', borderRadius:3, stack:'s'}}
                                    ]
                                  }},
                                  options: {{
                                    ...
                                    scales:{{
                                      ...
                                      y:{{stacked:true, grid:{{display:false}}, ticks:{{font:{{size:11}},color:'#475569'}}}}
                                    }}
                                  }}
                                }});
                              }}
                            ...
                            st.markdown("""<div style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:40px;text-align:center;">
                            <div style="font-size:13px;color:#94a3b8;">발송 데이터가 있는 Journey가 없습니다.</div></div>""", unsafe_allow_html=True)
                    ...
                    with donut_col:
                        ...
                        st.markdown(f"""<div style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:16px;">
                        <div style="font-size:13px;font-weight:500;color:#1e293b;margin-bottom:12px;">바운스율 분포</div>
                        <div style="position:relative;height:180px;">
                        <canvas id="donutChart"></canvas>
                        </div>
                        <div style="margin-top:12px;display:flex;flex-direction:column;gap:5px;font-size:11px;">
                        ...
                        (function() {{
                          function drawDonut() {{
                            ...
                            window._donutChart = new Chart(el, {{
                              type: 'doughnut',
                              data: {{
                                labels: ['정상','관찰','주의','위험'],
                                datasets: [{{
                                  data: [{normal},{watch},{warn},{danger}],
                                  backgroundColor: ['#dcfce7','#dbeafe','#fef9c3','#fee2e2'],
                                  borderColor: ['#bbf7d0','#bfdbfe','#fde68a','#fecaca'],
                                  borderWidth: 1
                                }}]
                              }},
