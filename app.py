...
WHERE
    ...
    if st.session_state.get("generated_sql"):
        ...
        sql_h = max(120, sql.count('\n') * 22 + 60)
        st.markdown(f"""
        <div style="position:relative;">
        <textarea id="sql_out" readonly onclick="copySql(this)"
        style="width:100%;height:{sql_h}px;
        background:#1e1e2e;color:#cdd6f4;border:0.5px solid #313244;border-radius:8px;
        padding:14px 16px;font-family:'IBM Plex Mono',monospace;font-size:13px;
        line-height:1.7;resize:none;outline:none;cursor:pointer;box-sizing:border-box;"
        >{sql}</textarea>
        <span id="copy_toast" style="display:none;position:absolute;top:8px;right:12px;
        background:#22c55e;color:#fff;font-size:11px;font-weight:500;
        padding:3px 10px;border-radius:6px;">✅ 복사됨!</span>
        </div>
        <div style="font-size:11px;color:#94a3b8;margin-top:4px;">💡 클릭하면 자동 복사됩니다</div>
        <script>
        function copySql(el) {{
            var sql = {sql_json};
        ...
        function showToast() {{
            var t = document.getElementById('copy_toast');
