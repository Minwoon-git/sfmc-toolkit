                ...
                if stats:
                    ...
                    with chart_col:
                        if chart_stats:
                            ...
                            fig_bar.update_layout(
                                barmode='stack', margin=dict(l=0,r=0,t=30,b=40),
                                height=chart_height,
                                title=dict(text="Journey별 발송 현황", font=dict(size=13, color='#1e293b'), x=0),
                            ...
                            st.markdown(f"""<div style="background:#fff;border:0.5px solid #e2e8f0;
                            border-radius:10px;padding:40px;text-align:center;height:{chart_height}px;
                            display:flex;align-items:center;justify-content:center;">
                            <div style="font-size:13px;color:#94a3b8;">발송 데이터가 있는 Journey가 없습니다.</div>
                            </div>""", unsafe_allow_html=True)
                    ...
                    with donut_col:
                        ...
                        fig_donut.update_layout(
                            title=dict(text="바운스율 분포", font=dict(size=13, color='#1e293b'), x=0),
                            margin=dict(l=0,r=0,t=30,b=0),
                            height=chart_height,
                            plot_bgcolor='#fff', paper_bgcolor='#fff'
                        )
                        st.plotly_chart(fig_donut, use_container_width=True)
                        st.markdown(f"""<div style="display:flex;flex-direction:column;gap:5px;font-size:11px;margin-top:-10px;">
