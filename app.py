...
.tag-amber { background: #fef9c3; color: #92400e; border: 1px solid #fde68a; }
...
with st.sidebar:
    ...
    menu = st.radio(
        "",
        ["🗂️ Journey / Automation 관리", "💾 SQL 생성기", "🔍 에러 분석기"],
...
def require_key():
    ...
    <span style="color:#64748b; font-size:12px;">키는 저장되지 않으며 세션 중에만 사용됩니다.</span>
    </div>
    """, unsafe_allow_html=True)
                ...
                for de_name, fields in de_schemas.items():
                    if fields:
                        col_tags = " &nbsp;".join([
                            ...
                            f'<span style="color:#64748b; font-size:10px;">({f["type"]})</span>'
                            for f in fields
                        ])
                        st.markdown(f"""
                        ...
                        <span style="color:#64748b; font-size:10px; margin-left:6px;">({len(fields)}개 컬럼)</span><br>
                        {col_tags}
                        </div>
                        """, unsafe_allow_html=True)
                    else:
        ...
        with tab_j:
            ...
            if journeys_all:
                ...
                if sel:
                    ...
                    act_col = st.columns([1.5, 1, 1, 1, 1, 4])
                    with act_col[0]:
...
Draft 미배포, 오래된 Published Journey, 버전 이슈 등을 파악하고 개선 제안을 한국어로 해주세요.""",
        ...
        with tab_a:
                    ...
                    while True:
                        resp = requests.get(
                            ...
                            params={
                                "$pageSize": 50,
                                "$page": page,
                                "$fields": "id,name,key,description,status,lastRunTime,lastRunStatus,lastRunInstanceId,scheduleState,startDate,endDate"
                            }
                        )
                        if resp.status_code == 401:
                            st.error("토큰 만료 — 다시 연결해주세요.")
            ...
            STATUS_MAP_STR = {
                ...
                "Building":        ("Building","#1d4ed8","#dbeafe"),
                "PausedSchedule":  ("PausedSchedule","#92400e","#fef9c3"),
                "Paused":          ("PausedSchedule","#92400e","#fef9c3"),
                "Inactive":        ("Inactive","#64748b","#f1f5f9"),
                "Deleted":         ("Deleted","#dc2626","#fee2e2"),
                "Running":         ("Running","#1d4ed8","#dbeafe"),
                "Stopped":         ("Stopped","#dc2626","#fee2e2"),
                "Scheduled":       ("Scheduled","#15803d","#dcfce7"),
                "AwaitingTrigger": ("AwaitingTrigger","#15803d","#dcfce7"),
            ...
            def get_auto_status(a):
                # lastRunStatus 우선 확인 (실제 마지막 실행 결과)
                last_run_status = a.get("lastRunStatus")
                if last_run_status and last_run_status not in ("", None):
                    return STATUS_MAP_STR.get(last_run_status, (last_run_status, "#64748b", "#f1f5f9"))

                # 없으면 status 필드 확인
                raw = a.get("status", a.get("statusId", a.get("automationStatus", None)))
                if raw is None:
                    return "알 수 없음", "#64748b", "#f1f5f9"
                if isinstance(raw, int):
                    return STATUS_MAP.get(raw, ("알 수 없음","#64748b","#f1f5f9"))
                if isinstance(raw, str):
                    if raw.isdigit():
                        return STATUS_MAP.get(int(raw), ("알 수 없음","#64748b","#f1f5f9"))
                    return STATUS_MAP_STR.get(raw, (raw,"#64748b","#f1f5f9"))
                return "알 수 없음", "#64748b", "#f1f5f9"

            if automations:
                error_count = sum(1 for a in automations if get_auto_status(a)[0] in ["Stopped","Deleted","Error","오류"])
                running_count = sum(1 for a in automations if get_auto_status(a)[0] in ["Running","Scheduled","AwaitingTrigger"])
                paused_count = sum(1 for a in automations if get_auto_status(a)[0] in ["PausedSchedule"])

                am1, am2, am3, am4 = st.columns(4)
                with am1:
                    st.markdown(f"""<div style="background:#f1f5f9;border:1px solid #2a2a4e;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#94a3b8;margin-bottom:6px;">전체</div>
                    <div style="font-size:28px;font-weight:600;color:#1e293b;">{len(automations)}</div></div>""", unsafe_allow_html=True)
                with am2:
                    st.markdown(f"""<div style="background:#dbeafe;border:1px solid #bfdbfe;border-radius:8px;padding:14px 16px;">
                ...
                st.markdown(f"<div style='font-size:12px;color:#64748b;margin:4px 0 8px 0;'>{len(filtered_autos)}개 표시 중 (필터 결과 {filtered_total_auto}개 / 전체 {len(automations)}개)</div>", unsafe_allow_html=True)

                st.markdown("---")

                # 헤더
                h1, h2, h3 = st.columns([4, 2, 2])
                with h1: st.markdown("<div style='font-size:11px;font-weight:500;color:#64748b;letter-spacing:0.05em;text-transform:uppercase;padding:4px 0;color:#64748b;'>이름</div>", unsafe_allow_html=True)
                with h2: st.markdown("<div style='font-size:11px;font-weight:500;color:#64748b;letter-spacing:0.05em;text-transform:uppercase;padding:4px 0;color:#64748b;'>상태</div>", unsafe_allow_html=True)
                with h3: st.markdown("<div style='font-size:11px;font-weight:500;color:#64748b;letter-spacing:0.05em;text-transform:uppercase;padding:4px 0;color:#64748b;'>마지막 실행</div>", unsafe_allow_html=True)
                ...
                for a in filtered_autos:
                    ...
                    with nc:
                        ...
                        <div style="font-size:13px;font-weight:500;color:#1e293b;">{name}</div>
                        <div style="font-size:11px;color:#64748b;margin-top:3px;">ID: {aid}</div>
                        </div>
                        """, unsafe_allow_html=True)
                    with bc:
                        st.markdown(f"""
