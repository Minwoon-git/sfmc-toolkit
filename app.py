...
/* 기본 버튼 — 파란색 (SFMC 연결, Journey 불러오기, 전체선택/해제 등) */
.stButton > button {
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("""
    <div style="font-size:11px; color:#c8c8e0; line-height:1.8;">
                ...
                for de_name, fields in de_schemas.items():
                    if fields:
                        col_tags = " &nbsp;".join([
                            ...
                            f'<span style="color:#aaaacc; font-size:10px;">({f["type"]})</span>'
                            for f in fields
                        ])
                        st.markdown(f"""
                        ...
                        <span style="color:#aaaacc; font-size:10px; margin-left:6px;">({len(fields)}개 컬럼)</span><br>
                        {col_tags}
                        </div>
                        """, unsafe_allow_html=True)
                    else:
    ...
    if not st.session_state.get("sfmc_token"):
        ...
        with tab_j:
            ...
            if journeys_all:
                ...
                with m1:
                    st.markdown(f"""<div style="background:#1a1a2e;border:1px solid #2a2a4e;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#8888aa;margin-bottom:6px;">전체</div>
                    <div style="font-size:28px;font-weight:600;color:#e8e8f0;">{len(journeys_all)}</div></div>""", unsafe_allow_html=True)
                with m2:
                    st.markdown(f"""<div style="background:#0a2015;border:1px solid #1a4030;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#3acc7a;margin-bottom:6px;">Published</div>
                    <div style="font-size:28px;font-weight:600;color:#3acc7a;">{status_counts.get("Published",0)}</div></div>""", unsafe_allow_html=True)
                with m3:
                    st.markdown(f"""<div style="background:#201500;border:1px solid #402a00;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#f0a030;margin-bottom:6px;">Draft</div>
                    <div style="font-size:28px;font-weight:600;color:#f0a030;">{status_counts.get("Draft",0)}</div></div>""", unsafe_allow_html=True)
                with m4:
                    st.markdown(f"""<div style="background:#0a1a30;border:1px solid #1a3a60;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#4a9eff;margin-bottom:6px;">Paused</div>
                    <div style="font-size:28px;font-weight:600;color:#4a9eff;">{status_counts.get("Paused",0)}</div></div>""", unsafe_allow_html=True)
                with m5:
                    st.markdown(f"""<div style="background:#200a0a;border:1px solid #401010;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#ff5555;margin-bottom:6px;">Stopped</div>
                    <div style="font-size:28px;font-weight:600;color:#ff5555;">{status_counts.get("Stopped",0)}</div></div>""", unsafe_allow_html=True)
                ...
                st.markdown(f"<div style='font-size:12px;color:#aaaacc;margin:6px 0;'>{len(journeys)}개 표시 중 (필터 결과 {filtered_total}개 / 전체 {len(journeys_all)}개)</div>", unsafe_allow_html=True)
                ...
                with hc1: st.markdown("<div style='font-size:11px;color:#aaaacc;'></div>", unsafe_allow_html=True)
                with hc2: st.markdown("<div style='font-size:11px;font-weight:500;color:#aaaacc;letter-spacing:0.05em;text-transform:uppercase;'>이름 / ID</div>", unsafe_allow_html=True)
                with hc3: st.markdown("<div style='font-size:11px;font-weight:500;color:#aaaacc;letter-spacing:0.05em;text-transform:uppercase;'>상태</div>", unsafe_allow_html=True)
                with hc4a: st.markdown("<div style='font-size:11px;font-weight:500;color:#aaaacc;letter-spacing:0.05em;text-transform:uppercase;'>일시정지</div>", unsafe_allow_html=True)
                with hc4b: st.markdown("<div style='font-size:11px;font-weight:500;color:#aaaacc;letter-spacing:0.05em;text-transform:uppercase;'>중단</div>", unsafe_allow_html=True)
                with hc4c: st.markdown("<div style='font-size:11px;font-weight:500;color:#aaaacc;letter-spacing:0.05em;text-transform:uppercase;'>삭제</div>", unsafe_allow_html=True)
                with hc5: st.markdown("<div style='font-size:11px;font-weight:500;color:#aaaacc;letter-spacing:0.05em;text-transform:uppercase;'>바로가기</div>", unsafe_allow_html=True)
                ...
                for idx, j in enumerate(journeys):
                    ...
                    with info_col:
                        ...
                        <div style="font-size:11px;color:#9999bb;margin-top:3px;">v{version} &nbsp;·&nbsp; 생성: {created} &nbsp;·&nbsp; 수정: {modified}</div>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        with tab_a:
            ...
            if automations:
                ...
                with am1:
                    st.markdown(f"""<div style="background:#1a1a2e;border:1px solid #2a2a4e;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#8888aa;margin-bottom:6px;">전체</div>
                    <div style="font-size:28px;font-weight:600;color:#e8e8f0;">{len(automations)}</div></div>""", unsafe_allow_html=True)
                with am2:
                    st.markdown(f"""<div style="background:#0a1a30;border:1px solid #1a3a60;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#4a9eff;margin-bottom:6px;">실행 중</div>
                    <div style="font-size:28px;font-weight:600;color:#4a9eff;">{running_count}</div></div>""", unsafe_allow_html=True)
                with am3:
                    st.markdown(f"""<div style="background:#201500;border:1px solid #402a00;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#f0a030;margin-bottom:6px;">일시정지</div>
                    <div style="font-size:28px;font-weight:600;color:#f0a030;">{paused_count}</div></div>""", unsafe_allow_html=True)
                with am4:
                    st.markdown(f"""<div style="background:#200a0a;border:1px solid #401010;border-radius:8px;padding:14px 16px;">
                    <div style="font-size:11px;color:#ff5555;margin-bottom:6px;">오류/중단</div>
                    <div style="font-size:28px;font-weight:600;color:#ff5555;">{error_count}</div></div>""", unsafe_allow_html=True)
                ...
                with h1: st.markdown("<div style='font-size:11px;font-weight:500;color:#555577;letter-spacing:0.05em;text-transform:uppercase;padding:4px 0;color:#aaaacc;'>이름</div>", unsafe_allow_html=True)
                with h2: st.markdown("<div style='font-size:11px;font-weight:500;color:#555577;letter-spacing:0.05em;text-transform:uppercase;padding:4px 0;color:#aaaacc;'>상태</div>", unsafe_allow_html=True)
                with h3: st.markdown("<div style='font-size:11px;font-weight:500;color:#555577;letter-spacing:0.05em;text-transform:uppercase;padding:4px 0;color:#aaaacc;'>마지막 실행</div>", unsafe_allow_html=True)
                ...
                for a in filtered_autos:
                    ...
                    with nc:
                        ...
                        <div style="font-size:13px;font-weight:500;color:#e8e8f0;">{name}</div>
                        <div style="font-size:11px;color:#9999bb;margin-top:3px;">ID: {aid}</div>
                        </div>
                        """, unsafe_allow_html=True)
                    with bc:
                        st.markdown(f"""
