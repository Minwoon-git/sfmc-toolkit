                ...
                for de_name, fields in de_schemas.items():
                    if fields:
                        col_tags = "".join([
                            ...
                            padding:4px 10px;border-radius:6px;border:0.5px solid #e2e8f0;
                            background:#f8fafc;font-size:12px;margin:3px;">
                            <span style="font-weight:500;color:#1e293b;">{f["name"]}</span>
                            <span style="font-size:10px;color:#94a3b8;">{f["type"]}</span>
                            {"<span style='font-size:9px;font-weight:500;color:#185FA5;background:#E6F1FB;padding:1px 5px;border-radius:3px;'>PK</span>" if f["pk"] else ""}
                            </span>'''
                            for f in fields
                        ])
                        st.markdown(f"""
